...
- Create a new worksheet in Snowsight and copy SQL statements in [setup.sql](src/setup.sql) into the worksheet and run it to create various objects (including db, schema, warehouse, tables, etc.) and load data using Snowsight using the provided .csv files (see setup.sql for details).
